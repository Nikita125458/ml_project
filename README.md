...
В качестве третьей модели используется классический алгоритм Логистической регрессии, который является линейным классификатором и хорошо подходит для задач бинарной классификации.
Используемые параметры:
- random_state: фиксированное значение (например, 42) для воспроизводимости результатов;
- max_iter: максимальное количество итераций для сходимости оптимизатора (значение берется из переменной окружения MODEL_PARAMETER_2).
Обучение: модель обучается на предобработанных тренировочных данных (X_train, y_train).
Предсказание: генерируются предсказания для тестовых данных (X_test), где целевой столбец отсутствует.
Оценка: точность модели оценивается на валидационной выборке (X_val, y_val) с помощью accuracy_score и выводится в лог.